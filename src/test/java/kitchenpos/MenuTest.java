...
public class MenuTest {
    ...
    class MenuCreateTest {
        ...
        void createMenuTest() {
            ...
            var menuGroup = createMenuGroup();
            menuGroupRepository.save(menuGroup);
            var menuRequest = createMenu();

            //when
            var sut = menuService.create(menuRequest);
        ...
        void invalidMenuAmount() {
            //상품명이나 가격이 없을때 에러처리
            var menu = createMenu(후라이드치킨_MINUS_PRICE);
        ...
        void invalidMenuProduct() {
            var menuGroup = createMenuGroup(한마리메뉴_MENU_GROUP_NAME, 후라이드치킨_MENU_GROUP_UUID);
            menuGroupRepository.save(menuGroup);
            var menu = createMenu(후라이드치킨_MENU_UUID, 후라이드치킨_MENU_NAME, 후라이드치킨_DEFAULT_PRICE, menuGroup, 후라이드치킨_MENU_GROUP_UUID);
            assertThatIllegalArgumentException().isThrownBy(() -> menuService.create(menu));
        }
        ...
        void menuProductNotEqualsProductSize() {
            ...
            var menu = createMenu(menuGroup, menuProducts);
            assertThatIllegalArgumentException().isThrownBy(() -> menuService.create(menu));
        }
        ...
        void invalidTotalMenuAmount() {
            ...
            var menu = createMenu(후라이드치킨_OVER_PRICE);
        ...
        void invalidMenuName() {
            var product = createProduct(MenuTest.후라이드치킨_PRODUCT_UUID, PRODUCT_NAME_비속어, 후라이드치킨_DEFAULT_PRICE);
            var menuGroup = createMenuGroup(한마리메뉴_MENU_GROUP_NAME, 후라이드치킨_MENU_GROUP_UUID);
            menuGroupRepository.save(menuGroup);
            var menuProducts = List.of(createMenuProduct(product, 1, 후라이드치킨_PRODUCT_UUID));
            var menu = createMenu(후라이드치킨_MENU_UUID, 후라이드치킨_PROFANITY_MENU_NAME, 후라이드치킨_DEFAULT_PRICE, menuGroup, menuProducts, 후라이드치킨_MENU_GROUP_UUID);
    ...
    class MenuChangePriceTest {
        ...
        void changeMenu() {
            createAndSaveMenu();
            var menuRequest = createMenu(new BigDecimal(19000));
            var menu = menuService.changePrice(후라이드치킨_MENU_UUID, menuRequest);
        ...
        @Test
        void invalidMenuAmount() {
            var menuRequest = createMenu(new BigDecimal(-10));
        ...
        void invalidTotalMenuAmount() {
            createAndSaveMenu();
            var menuRequest = createMenu(후라이드치킨_OVER_PRICE);
            assertThatIllegalArgumentException().isThrownBy(() -> menuService.changePrice(후라이드치킨_MENU_UUID, menuRequest));
        }
    ...
    class MenuDisplayTest {
        ...
        void displayMenu() {
            createAndSaveMenu();
            var ResponseMenu = menuService.display(후라이드치킨_MENU_UUID);
    ...
    class MenuHideTest {
        ...
        void displayMenu() {
            ...
            var ResponseMenu = menuService.hide(후라이드치킨_MENU_UUID);
    ...
    class findAllMenuTest {
        ...
        void displayMenu() {
            createAndSaveMenu(후라이드치킨_MENU_UUID, 후라이드치킨_MENU_NAME, 후라이드치킨_DEFAULT_PRICE);
            var ResponseMenu = menuService.findAll();
