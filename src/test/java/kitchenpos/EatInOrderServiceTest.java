...
import static kitchenpos.fixture.EatInOrderFixture.createOrder;
import static kitchenpos.fixture.EatInOrderLineItemFixture.createOrderLineItem;
import static kitchenpos.fixture.EatInOrderTableFixture.createOrderTable;
import static kitchenpos.order.eatinorder.domain.EatInOrderStatus.*;
...
public class EatInOrderServiceTest {
    ...
    class OrderCreateTest {
        ...
        void validateStateWaiting() {
            var menuId = menuRepository.save(createMenu(후라이드치킨_DEFAULT_PRICE, true, createMenuProduct())).getId();
            var orderTable = createOrderTable(ORDER_TABLE_ID, ORDER_TABLE_NAME, 0, true);
            orderTableRepository.save(orderTable);
            var orderRequest = createOrder(ORDER_UUID, ORDER_TYPE_매장내식사주문, ORDER_STATUS_주문대기, ORDER_DATE_TIME_주문요청시간,
        ...
        void invalidOrderType() {
            var order = createOrder(ORDER_UUID, ORDER_TYPE_미선택, ORDER_STATUS_주문대기, ORDER_DATE_TIME_주문요청시간,
        ...
        void invalidOrderLineItems() {
            var order = createOrder(ORDER_TYPE_미선택);
            assertThatIllegalArgumentException().isThrownBy(() -> orderService.create(order));
        }
        ...
        void notEqualMenuAndOrderLineItemSize() {
            var orderLineItem = createOrderLineItem((UUID) null, DEFAULT_QUANTITY, 후라이드치킨_DEFAULT_PRICE);
            var order = createOrder(orderLineItem, "강남구");
            assertThatIllegalArgumentException().isThrownBy(() -> orderService.create(order));
        }
        ...
        void invalidOrderLineItemSize() {
            var orderLineItem = createOrderLineItem(후라이드치킨_MENU_UUID, MINUS_QUANTITY, 후라이드치킨_DEFAULT_PRICE);
            var order = createOrder(orderLineItem, "강남구");
            assertThatIllegalArgumentException().isThrownBy(() -> orderService.create(order));
        }
        ...
        void selectedMenuNotDisplayed() {
            ...
            var order = createOrder(createOrderLineItem(menuId), "강남구");
        ...
        void notEqualMenuAndOrderMenuPrice() {
            var orderLineItem = createOrderLineItem(후라이드치킨_MENU_UUID, DEFAULT_QUANTITY, 후라이드치킨_OVER_PRICE);
        ...
        void validateEatInOrderTable() {
            var menuId = menuRepository.save(createMenu(후라이드치킨_DEFAULT_PRICE, false, createMenuProduct())).getId();
            var orderTable = createOrderTable(ORDER_TABLE_ID, ORDER_TABLE_NAME, 0, TABLE_UNUSABLE);
            orderTableRepository.save(orderTable);
            var order = createOrder(ORDER_UUID, ORDER_TYPE_매장내식사주문, ORDER_STATUS_주문대기, ORDER_DATE_TIME_주문요청시간,
                    createOrderLineItem(menuId), "강남구", orderTable, orderTable.getId());
            assertThatIllegalStateException().isThrownBy(() -> orderService.create(order));
        }
    }
    ...
    class OrderAcceptTest {
        ...
        void validateStateWaiting() {
            ...
            var orderRequest = createOrder(createOrderLineItem(menu));
            orderRepository.save(orderRequest);
            var orderResponse = orderService.accept(orderRequest.getId());
    ...
    class OrderServedTest {
        ...
        void validateStateAccepted() {
            var menu = createMenu(후라이드치킨_DEFAULT_PRICE, true, createMenuProduct());
            menuRepository.save(menu);
            var orderRequest = createOrder(createOrderLineItem(menu), ORDER_STATUS_주문대기);
            orderRepository.save(orderRequest);
        ...
        void validateStateServed() {
            var menu = createMenu(후라이드치킨_DEFAULT_PRICE, true, createMenuProduct());
            menuRepository.save(menu);
            var orderRequest = createOrder(createOrderLineItem(menu), ORDER_STATUS_주문수락);
            orderRepository.save(orderRequest);
    ...
    class OrderStartDeliveryTest {
        ...
        void invalidOrderStatus() {
            var menu = createMenu(후라이드치킨_DEFAULT_PRICE, true, createMenuProduct());
            menuRepository.save(menu);
            var orderRequest = createOrder(createOrderLineItem(menu), ORDER_TYPE_매장내식사주문, ORDER_STATUS_주문대기);
            orderRepository.save(orderRequest);
        ...
        void invalidStateServed() {
            var menu = createMenu(후라이드치킨_DEFAULT_PRICE, true, createMenuProduct());
            menuRepository.save(menu);
            var orderRequest = createOrder(createOrderLineItem(menu), ORDER_TYPE_배달주문, ORDER_STATUS_주문대기);
            orderRepository.save(orderRequest);
            assertThatIllegalStateException().isThrownBy(() -> orderService.startDelivery(orderRequest.getId()));
        }
        ...
        void validateStateServed() {
            var menu = createMenu(후라이드치킨_DEFAULT_PRICE, true, createMenuProduct());
            menuRepository.save(menu);
            var orderRequest = createOrder(createOrderLineItem(menu), ORDER_TYPE_배달주문, ORDER_STATUS_제공완료);
            orderRepository.save(orderRequest);
...
/*
    ...
    class OrderCompleteDeliveryTest {
        ...
        void validateStateDelivered() {
            var menu = createMenu(후라이드치킨_DEFAULT_PRICE, true, createMenuProduct());
            menuRepository.save(menu);
            var orderRequest = createOrder(createOrderLineItem(menu), ORDER_TYPE_배달주문, ORDER_STATUS_배달중);
            orderRepository.save(orderRequest);
    ...
    class OrdercompleteTest {
       /* @DisplayName(value = "배달인 주문일 경우 현재 상태가 배달 완료(DELIVERED)여야 합니다.")
        ...
        @Test
        void validateNotServedTypeState() {
            var menu = createMenu(후라이드치킨_DEFAULT_PRICE, true, createMenuProduct());
            menuRepository.save(menu);
            var orderRequest = createOrder(createOrderLineItem(menu),ORDER_TYPE_매장내식사주문, ORDER_STATUS_주문완료);
            orderRepository.save(orderRequest);
            assertThatIllegalStateException().isThrownBy(() -> orderService.complete(orderRequest.getId()));
        }
        ...
        void eatInStateClearTable() {
            ...
            orderTableRepository.save(orderTable);
            var orderRequest = createOrder(createOrderLineItem(menu), ORDER_TYPE_매장내식사주문, ORDER_STATUS_제공완료, orderTable);
            orderRepository.save(orderRequest);
